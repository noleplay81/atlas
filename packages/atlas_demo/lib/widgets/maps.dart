...
class _MapState extends State<Map> {
    ...
    return BlocConsumer<MapBloc, MapState>(
      ...
      listener: (context, state) {
        if (state.currentCameraPosition.type == CameraPositionType.Center) {
          atlasController.moveCamera(_toCameraPosition(state.currentCameraPosition));
        } else {
          atlasController.updateBounds(_toLatLngBounds(state.currentCameraPosition), 0);
        }
      },
      builder: (context, state) {
        return Atlas(
          ...
          onLongPress: (coordinates) => state.notifyLongPressedOnMap
              ...
              : null,
          onCameraPositionChanged: (position) => state.notifyCameraPositionChange
  ...
  LatLng _toLatLng(DemoGeoCoordinates source) =>
      (source == null) ? null : LatLng(latitude: source.latitude, longitude: source.longitude);
  ...
  CameraPosition _toCameraPosition(DemoCameraPosition source) =>
      (source == null || source.type != CameraPositionType.Center)
          ? null
          : CameraPosition(target: _toLatLng(source.center), zoom: source.zoom.toDouble());

  LatLngBounds _toLatLngBounds(DemoCameraPosition source) =>
      (source == null || source.type != CameraPositionType.Bound)
          ? null
          : LatLngBounds(
              northeast: LatLng(latitude: source.topLeft.latitude, longitude: source.bottomRight.longitude),
              southwest: LatLng(latitude: source.bottomRight.latitude, longitude: source.topLeft.longitude),
            );
              ...
              .map((demoMarker) => !selectedIds.contains(demoMarker.id)
                  ...
                  : Marker(
                      ...
                      icon: (demoMarker.icon == null)
                          ? null
                          : MarkerIcon(assetName: demoMarker.icon),
                      position: _toLatLng(demoMarker.position),
                      anchor: Anchor(x: demoMarker.anchor?.x ?? 0.5, y: demoMarker.anchor?.y ?? 0.5),
